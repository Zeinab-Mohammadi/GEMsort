...
# this function calculates the distance between the new datapoint and data64 and also calculates the distance treshold 
...
def newdata_dis(GEMsort_C, GEMsort_w, N_all, onedata_new, nodes_total1, path):
    ...
    X2 = res2 #these are data with 64 length in every row
    #y = label #these are labels: 0, 1, 2
    n_components =2
    ...
    ################################################################
    #calculating the th1 (the treshold of the distance for the new data and other data64 points)
        diss=[]
        ...
        #X1 =N 
        if minval< th1:
            Group=Group_num  #since number of group starts from 1 but G starts from 0
