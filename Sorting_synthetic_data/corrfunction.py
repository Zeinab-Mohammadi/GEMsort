...
def corr(X, ch_num, T1 , Tp, Tdis, T):
    ...
    #stage 2: finding the similar peaks in near times and near channels
        ...
        for i in range (p_all.shape[0]): 
            for j in range (p_all[i].shape[0]):
                if (p_all[i][j] != 0 and np.abs(p_all[i][j] - rowp[0,k]) < 2 and X[i,rowp[0,k]] != 0): 
                    rows[k].append(i) # for all the channels number with the same peak 
                    x = X[i, 0:T]
                    signals[k, i, :] = cut(x,p_all[i][j],T1)  
